...
List EDM_multi_a(const NumericVector& Z, int min_size=24, int min_rsize=12, double beta=0, int degree=0){
	...
	vector<int> start(n+1,0);//store optimal location of the start of previous change point box
	...
	for(int s=min_size + min_rsize; s<n+1; ++s){
		...
		for(int t=min_size; t<s-min_rsize+1; ++t){//modify limits to deal with min_size
